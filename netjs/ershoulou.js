		...
		function inArray(arr,value){
			for(var i=0;i<arr.length;i++){
				if(arr[i].id==value&&arr[i].type=="3"){
					return true;
				}
			}
			return false;
		}
		$.post("action.php", {action:"getshhlist"}, function(data){//alert(data);
			var datas = jQuery.parseJSON(data);
			//var nav_a = $("#nav").find("a").size(); 
			if(datas.success == 1){
				var arr = datas.data;
				var html = "";
				for(var i=0; i<arr.length; i++){
				...
				$(".gouwuche").click(function(){
					...
					if(typeof(Storage) != "undefine"){
						if(!inArray(cp_id,$(this).attr("name"))){
							...
							var op = {id:$(this).attr("name"),type:3,img:$(this).attr("data1"),title:$(this).attr("data2")};
							cp_id.push(op);
