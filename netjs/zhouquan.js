		...
		function inArray(arr,value){
			for(var i=0;i<arr.length;i++){
				if(arr[i].id==value&&arr[i].type=="2"){
					return true;
				}
			}
			return false;
		}
		$.post("action.php", {action:"getmorlist"}, function(data){//alert(data);
				...
				$(".gouwuche").click(function(){
					...
					if(typeof(Storage) != "undefine"){
						if(!inArray(cp_id,$(this).attr("name"))){
							...
							var op = {id:$(this).attr("name"),type:2,img:$(this).attr("data1"),title:$(this).attr("data2")};
							cp_id.push(op);
